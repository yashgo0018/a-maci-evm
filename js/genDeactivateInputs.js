...
const allDeactivates = JSON.parse(fs.readFileSync(
  path.join(outputPath, `deactivates.json`)
).toString())
const activeStates = JSON.parse(fs.readFileSync(
  path.join(outputPath, `active.json`)
).toString()).map((t) => BigInt(t))
...
for (const state of logs.states) {
  main.initStateTree(Number(state.idx), toBigInt(state.pubkey), state.balance, state.c)
}
...
while (main.processedDMsgCount < logs.dmessages.length) {
  ...
  const { input, newDeactivate } = main.processDeactivateMessage(size, Number(logs.dmessages[i - 1].numSignUps))

  fs.writeFileSync(
    path.join(outputPath, `d-input_${jsonIdx.toString().padStart(4, '0')}.json`),
    JSON.stringify(stringizing(input), undefined, 2)
  )
  fs.writeFileSync(
    path.join(outputPath, `deactivates_${jsonIdx.toString().padStart(4, '0')}.json`),
    JSON.stringify(stringizing({ deactivates: newDeactivate, size }), undefined, 2)
  )
  jsonIdx++
